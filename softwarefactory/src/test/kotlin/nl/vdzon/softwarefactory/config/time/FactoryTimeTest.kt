...
/**
 * Tests voor de NL↔UTC-conversie van [FactoryTime]. DST-correctheid wordt getoetst met een vaste klok:
 * dezelfde wandklok-tijd in NL valt 's winters op een ander UTC-moment dan 's zomers.
 */
class FactoryTimeTest {

    private fun at(instant: String): FactoryTime =
        FactoryTime(Clock.fixed(Instant.parse(instant), ZoneOffset.UTC))

    @Test
    fun `NL-tijd in winter is UTC+1`() {
        // 15 jan 2026: CET (UTC+1) → 02:00 NL = 01:00 UTC.
        val instant = FactoryTime(Clock.systemUTC())
    ...
    fun `parseHhMm en formatHhMm zijn elkaars inverse`() {
        assertEquals("02:00", FactoryTime.formatHhMm(FactoryTime.parseHhMm("02:00")))
        assertEquals(LocalTime.of(7, 30), FactoryTime.parseHhMm("07:30"))
    }
}
