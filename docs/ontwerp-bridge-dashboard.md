...
| Operatie | Delegeert naar |
|---|---|
| `story.create` | `createStory(project, title, description, repo, aiSupplier, aiModel, start, questionsAllowed, approvalMode, notificationEvents, hotfix)` — `notificationEvents` bevat alleen concrete eventnamen en valt bij ontbreken terug op `DEPLOYED`, `QUESTION`, `MANUAL_ACTION_REQUIRED`, `ERROR`. `hotfix` is optioneel (default `false`); staat het aan, dan krijgt de story alleen `hotfix`, `merge`, `deploy`. `projectKey` is optioneel en valt terug op het enige project. |
...
| `story.setNotificationEvents` | `setNotificationEvents(key, events)` — `POST /api/v1/stories/{storyKey}/notification-events`; accepteert iedere combinatie van de acht concrete events, inclusief leeg. Story-only; subtaken lezen de parentset. |
