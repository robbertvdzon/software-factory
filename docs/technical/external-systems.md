...
- Historie van álle opruimrondes opslaan en uitlezen (`maintenance_cleanup_runs`,
  `V30__maintenance_cleanup_runs.sql` + `V31__maintenance_cleanup_kinds.sql` +
  `V32__maintenance_cleanup_trigger.sql`): één rij per ronde met een `kind`-kolom, een nullable
  `project` en een `trigger` (`scheduled`/`manual`, SF-1929), nieuwste eerst voor het dashboard en
  optioneel op project en/of soort gefilterd, met eigen retentie via `deleteOlderThan`.
