...
| `softwarefactory/.../dashboard/services/FactoryOperationsService.kt` (inmiddels verhuisd uit `web/services/`) | Implementeert de poort `core/FactoryOperations`; heeft `setStoryPhase(key, phase, comment)`, `setSubtaskPhase(key, phase, comment)`, `queueCommand(key, FactoryCommand, reason)`, en de leesmethodes `testerReportFor(key)` / `deploySummaryFor(key)` (SF-1830, de functionele samenvatting voor de deploymelding). |
...
| `story.setNotificationEvents` | `setNotificationEvents(key, events)` — `POST /api/v1/stories/{storyKey}/notification-events`; accepteert iedere combinatie van de acht exacte concrete events, inclusief leeg. Een onbekende naam geeft `INVALID_PARAMS` zonder de bestaande set te overschrijven. Story-only; subtaken lezen de parentset en een falende parent-resolutie onderdrukt meldingen fail-closed. |
