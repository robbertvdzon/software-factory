...
`setNotificationEvents(...)`, `saveNightlySettings(...)`, `purgeStory(key)`, `startRefining(key)`, `startDeveloping(key)`, `forceProjectDeploy(name)`, `openWorkspaceInIntellij(key)`. **De bridge hergebruikt deze methodes — schrijf geen nieuwe businesslogica.** |
| `softwarefactory/.../dashboard/services/FactoryOperationsService.kt` (inmiddels verhuisd uit `web/services/`) | Implementeert de poort `core/FactoryOperations`; heeft `setStoryPhase(key, phase, comment)`, `setSubtaskPhase(key, phase, comment)`, `queueCommand(key, FactoryCommand, reason)`, en de leesmethode `testerReportFor(key)`. |
...
| `agents.list` / `projects.list` / `nightly.get` / `settings.get` | `agents()` / `projectsOverview()` / `nightlyJobs(run)` / `settings()` — **SF-1288:** `merged.list`/`rollout.list` en de bijbehorende `merged()`/`rollout()`-methodes zijn verwijderd (Merged/Rollout-tabs en de uitlog-knop kwamen te vervallen). **SF-890:** `projectsOverview()` levert per project sindsdien ook `buildStatus` (laatste main-build-tijdstip, main/PR-build-actief, in-sync/out-of-sync t.o.v. `prdVersion`), afgeleid uit dezelfde `GitHubActionsClient` als `builds.list` hieronder; geen nieuwe operatie of endpoint. |
...
| `changelog.for` | `changelogFor(name)` (`BridgeRequestHandler.kt:113` → `dashboardService.changelogFor(name)`, parameter `name`) — per project de `shortDescriptionSummary` van elke story die er een heeft, nieuwste eerst. Dit is de geauthenticeerde route die de eigen dashboard-frontend gebruikt; het publieke `GET /api/v1/public/changelog/{projectName}` op de factory levert dezelfde inhoud auth-vrij voor andere apps. |
...
| Operatie | Delegeert naar |
|---|---|
| `story.create` | `createStory(project, title, description, repo, aiSupplier, aiModel, start, questionsAllowed, approvalMode, notificationEvents, hotfix)` — `notificationEvents` bevat alleen exacte concrete eventnamen en valt bij ontbreken terug op `DEPLOYED`, `QUESTION`, `MANUAL_ACTION_REQUIRED`, `ERROR`; een onbekende naam geeft `INVALID_PARAMS` zonder story-write. `hotfix` is optioneel (default `false`); staat het aan, dan krijgt de story alleen `hotfix`, `merge`, `deploy`. `projectKey` is optioneel en valt terug op het enige project. |
...
| `story.setNotificationEvents` | `setNotificationEvents(key, events)` — `POST /api/v1/stories/{storyKey}/notification-events`; accepteert iedere combinatie van de acht exacte concrete events, inclusief leeg. Een onbekende naam of subtaak-key geeft `INVALID_PARAMS` vóór iedere write. Story-only; subtaken lezen de parentset en een falende parent-resolutie onderdrukt meldingen fail-closed. |
...
| `maintenance.cleanupsList` / `maintenance.cleanupDetail` | **SF-1913 / SF-1921:** `maintenanceCleanups(project?, kind?)` / `maintenanceCleanupDetail(id)` — gedeelde historie van álle opruimrondes (`maintenance_cleanup_runs`, migraties `V30`/`V31`), nieuwste eerst (max 200, geen paginering), optioneel op project en/of soort gefilterd. `GET /api/v1/maintenance/cleanups[?project=…][&kind=…]` en `GET /api/v1/maintenance/cleanups/{id}`; een onbekende id geeft `NOT_FOUND` (HTTP 404). **SF-1929:** de lijst geeft naast `runs` ook `runningKinds` terug (de soorten die nu draaien) en beide DTO's dragen `trigger` (`scheduled`/`manual`, migratie `V32`). **SF-1939:** de lijst geeft daarnaast `summary` terug — de laatste ronde per `kind` en, voor `github-releases`, per project, met dezelfde velden als een lijstregel. Die komt uit een eigen query (`MaintenanceCleanupRunRepository.latestPerKindAndProject()`) en niet uit de op 200 rijen afgekapte `runs`, zodat een drukke soort de laatste ronde van een rustige soort niet uit beeld duwt; `summary` wordt niet meegefilterd met `project`/`kind`. Het is een extra veld met default, dus een al uitgerolde APK blijft werken. |
| `maintenance.runNow` | **SF-1929:** `runCleanupNow(kind)` — start één opruimronde nu; `kind` is een `CleanupKinds`-waarde of `all`. Loopt via `runtime.CleanupRunNowApi` (de vier factory-brede opruimers) en `maintenance.MaintenanceCleanupApi` (de GitHub-cleanup): exact dezelfde ronde als de scheduler, niet-blokkerend op een executor gestart zodat de 30s-bridgetimeout een lange GitHub-ronde nooit afkapt. `POST /api/v1/maintenance/run`. Antwoord volgt het `audit.runNow`-precedent — HTTP 200 met `started` + `status` (`started`/`already_running`/`disabled`/`unknown_kind`) plus `kinds` (de status per soort, nodig voor `all`); een geweigerd verzoek is dus géén foutcode. Echte fouten (onbekende parameter, factory offline) blijven het `BridgeError`-pad volgen. |
