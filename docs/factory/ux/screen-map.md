...
| `/changelog/{project}` | Changelog | Per-project changelog (`ChangelogScreen`, SF-2086) on its own bookmarkable URL (SF-2087). The project name is URL-encoded. On web the changelog button in the projects list opens this address in a new tab (`url_launcher`, `webOnlyWindowName: '_blank'`); a cold load on the address shows the changelog as a standalone page (own `AppBar` titled `Changelog — <project>`, no app navigation and no back button) as soon as the session is valid — the deep link survives a fresh Google login. An unknown or empty project part falls back to the screen's own error/empty state. On the Android APK the button keeps its in-app `Navigator.push`. This is the only deep link; all other screens stay on the root URL. |
