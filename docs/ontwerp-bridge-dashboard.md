...
| `agent.log` | **SF-1010:** `DashboardQueryService.agentLog(agentRunId)` — de gecapturede `docker-stdout`/`docker-stderr`-regels (`AgentEventRepository.recentForAgentRun`, begrensd op de laatste 500) van één agent-run, via de geëxposeerde runtime-poort `AgentLogApi` (de `dashboard`-module mag `runtime.repositories.AgentEventRepository` niet direct injecteren, Spring-Modulith-grens). Levert ook `outcome`/`ended` (afgeleid van `agentRunById`) zodat de frontend stopt met pollen zodra de run een outcome heeft. Endpoint: `GET /api/v1/agents/{agentRunId}/log` in dashboard-backend. |
