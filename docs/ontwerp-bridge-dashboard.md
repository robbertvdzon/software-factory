...
| `maintenance.cleanupsList` / `maintenance.cleanupDetail` | **SF-1913:** `maintenanceCleanups(project?)` / `maintenanceCleanupDetail(id)` — historie van de nachtelijke release/package-cleanup (`maintenance_cleanup_runs`, migratie `V30`), nieuwste eerst (max 200, geen paginering), optioneel op project gefilterd. `GET /api/v1/maintenance/cleanups[?project=…]` en `GET /api/v1/maintenance/cleanups/{id}`; een onbekende id geeft `NOT_FOUND` (HTTP 404). |
