...
| `maintenance.cleanupsList` / `maintenance.cleanupDetail` | **SF-1913 / SF-1921:** `maintenanceCleanups(project?, kind?)` / `maintenanceCleanupDetail(id)` — gedeelde historie van álle opruimrondes (`maintenance_cleanup_runs`, migraties `V30`/`V31`), nieuwste eerst (max 200, geen paginering), optioneel op project en/of soort gefilterd. `GET /api/v1/maintenance/cleanups[?project=…][&kind=…]` en `GET /api/v1/maintenance/cleanups/{id}`; een onbekende id geeft `NOT_FOUND` (HTTP 404). **SF-1929:** de lijst geeft naast `runs` ook `runningKinds` terug (de soorten die nu draaien) en beide DTO's dragen `trigger` (`scheduled`/`manual`, migratie `V32`). |
