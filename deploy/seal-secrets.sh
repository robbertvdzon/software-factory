...
EOF
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    case "$key" in
      SF_YOUTRACK_BASE_URL|SF_YOUTRACK_TOKEN|SF_YOUTRACK_PROJECTS|SF_GITHUB_TOKEN|SF_DATABASE_URL|SF_DATABASE_SCHEMA|SF_DASHBOARD_USERNAME|SF_DASHBOARD_PASSWORD|SF_DASHBOARD_REMEMBER_SECRET|SF_DASHBOARD_COOKIE_SECURE)
        printf '  %s: |-\n' "$key"
        printf '%s\n' "$val" | sed 's/^/    /'
        count=$((count + 1))
        ;;
